...
class Topology {
 public:
    ...
    virtual int AddPhysicalPoolJustForTest(const PhysicalPool &data) = 0;
    ...
    virtual bool GetPhysicalPool(const std::string &poolName,
        const std::string &poolsetName, PhysicalPool *out) const = 0;

    virtual bool GetPhysicalPool(const std::string &poolName,
        PoolsetIdType poolsetId, PhysicalPool *out) const = 0;
...
class TopologyImpl : public Topology {
 public:
    ...
    // get physicalPool list
    std::list<PoolIdType>
        GetPhysicalPoolInPoolset(PoolsetIdType id,
        PhysicalPoolFilter filter = [](const PhysicalPool&) {
            return true;}) const override;
    // get zone list
    std::list<ZoneIdType>
