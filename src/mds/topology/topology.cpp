...
int TopologyImpl::RemovePoolset(PoolsetIdType id) {
    if (id == kDefaultPoolsetId) {
        LOG(WARNING) << "Default poolset is not allowed to be deleted";
...
int TopologyImpl::Init(const TopologyOption &option) {
    ...
    if (poolsetMap_.empty()) {
        const bool succ = CreateDefaultPoolset();
        if (!succ) {
            return kTopoErrCodeStorgeFail;
        }

        maxPoolsetId = kDefaultPoolsetId;
    }
    ...
    for (auto& phy : physicalPoolMap_) {
        auto pid = phy.second.GetPoolsetId();
        if (pid == UNINTIALIZE_ID) {
            // old cluster's physicals don't have corresponding poolset, we just
            // place them into the default poolset
            pid = curve::common::kDefaultPoolsetId;
            phy.second.SetPoolsetId(pid);
        }

        assert(pid != UNINTIALIZE_ID);
...
bool TopologyImpl::CreateDefaultPoolset() {
    assert(poolsetMap_.empty());

    Poolset data{kDefaultPoolsetId, kDefaultPoolsetName, kDefaultPoolsetName,
                 kDefaultPoolsetName};
    const bool succ = storage_->StoragePoolset(data);
    if (!succ) {
        LOG(WARNING) << "Create default poolset failed";
        return false;
    }
