...
#include "curvefs/test/space/mock/mock_space_manager.h"
...
TEST_F(SpaceAllocServiceTest, TestUnInitSpace) {
    // test failed
    {
        ...
        request.set_fsid(1);
        request.set_fstype(mds::FSType::TYPE_VOLUME);

        EXPECT_CALL(space_, UnInitSpace(_, _))
    ...
    // test success
    {
        ...
        EXPECT_CALL(space_, StatSpace(_, _, _, _))
            .WillOnce(DoAll(SetArgPointee<1>(100 * kGiB),
                            ...
                            SetArgPointee<3>(4 * kKiB), Return(SPACE_OK)));
...
TEST_F(SpaceAllocServiceTest, TestAllocateSpace) {
    ...
    // test success
    {
        ...
        Extents exts{{12 * kKiB, 8 * kKiB}};

        EXPECT_CALL(space_, AllocateSpace(_, _, _, _))
            .WillOnce(DoAll(SetArgPointee<3>(exts), Return(SPACE_OK)));
...
TEST_F(SpaceAllocServiceTest, TestAllocateSpaceWithHint) {
    // no hint
    {
        AllocateSpaceRequest request;
        AllocateSpaceResponse response;
        ...
        EXPECT_CALL(space_, AllocateSpace(1, 8 * kKiB, hint, _))
            .WillOnce(DoAll(SetArgPointee<3>(exts), Return(SPACE_OK)));
    ...
    // hint with allocate type
    {
        ...
        request.set_fsid(1);
        request.set_size(8 * kKiB);
        auto* protoHint = request.mutable_allochint();
        protoHint->set_alloctype(AllocateType::BIG);
    ...
    // hint with allocate type and left/right offset
    {
        AllocateSpaceRequest request;
        AllocateSpaceResponse response;
        ...
        auto* protoHint = request.mutable_allochint();
        protoHint->set_alloctype(AllocateType::BIG);
        protoHint->set_leftoffset(0 * kKiB);
        protoHint->set_rightoffset(8 * kKiB);
...
TEST_F(SpaceAllocServiceTest, TestDeallocateSpace) {
    ...
    // test success
    {
        ...
        google::protobuf::RepeatedPtrField<curvefs::space::Extent> protoExts;

        EXPECT_CALL(space_, DeallocateSpace(_, _))
            .WillOnce(DoAll(SaveArg<1>(&protoExts), Return(SPACE_OK)));
...
TEST_F(SpaceAllocServiceTest, TestAllocateS3Chunk) {
    // test failure
    {
        ...
        EXPECT_CALL(space_, AllocateS3Chunk(_, _))
            .WillOnce(Return(SPACE_NOT_FOUND));
    ...
    // test success
    {
        ...
        uint64_t chunkIdx = 100;
        EXPECT_CALL(space_, AllocateS3Chunk(_, _))
            .WillOnce(DoAll(SetArgPointee<1>(chunkIdx), Return(SPACE_OK)));
        ...
        EXPECT_FALSE(cntl.Failed());
        EXPECT_EQ(SPACE_OK, response.status());
        EXPECT_EQ(chunkIdx, response.chunkid());
    }
}
