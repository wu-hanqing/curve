...
SpaceStatusCode SpaceManagerImpl::UnInitSpace(uint32_t fsId,
                                              mds::FSType fsType) {
    return fsType == mds::FSType::TYPE_VOLUME ? UnInitBlockSpace(fsId)
                                              : UnInitS3Space(fsId);
}

SpaceStatusCode SpaceManagerImpl::StatSpace(uint32_t fsId, uint64_t* total,
                                            uint64_t* available,
                                            uint64_t* blkSize) {
...
SpaceStatusCode SpaceManagerImpl::AllocateS3Chunk(uint32_t fsId, uint64_t* id) {
...
SpaceStatusCode SpaceManagerImpl::InitS3Space(const mds::FsInfo& fsInfo) {
    ...
    std::unique_ptr<S3Allocator> allocator(new S3Allocator());

    if (FLAGS_enableReload) {
        S3Reloader reloader(allocator.get(), fsInfo.fsid(),
                            fsInfo.rootinodeid());
        if (!reloader.Reload()) {
    ...
    LOG(INFO) << "Init s3 allocator success, fsId: " << fsInfo.fsid();
    return SPACE_OK;
}

SpaceStatusCode SpaceManagerImpl::InitBlockSpace(const mds::FsInfo& fsInfo) {
    ...
    if (FLAGS_enableReload) {
        BlockVolumeRealoder reloader(allocator.get(), fsInfo.fsid(),
                                     fsInfo.rootinodeid());
...
SpaceStatusCode SpaceManagerImpl::UnInitS3Space(uint32_t fsId) {
    WriteLockGuard lock(s3Rwlock_);
    auto iter = s3Allocator_.find(fsId);
    if (iter == s3Allocator_.end()) {
        return SPACE_NOT_FOUND;
    ...
    s3Allocator_.erase(iter);
    return SPACE_OK;
}

SpaceStatusCode SpaceManagerImpl::UnInitBlockSpace(uint32_t fsId) {
    WriteLockGuard lock(blkRwlock_);
    auto iter = blkAllocators_.find(fsId);
    if (iter == blkAllocators_.end()) {
        return SPACE_NOT_FOUND;
