...
uint64_t BitmapAllocator::Alloc(uint64_t size, const SpaceAllocateHint& hint,
    ...
    } else if (hint.leftOffset == hint.INVALID_OFFSET &&
...
bool BitmapAllocator::DeAlloc(const Extents& exts) {
...
bool BitmapAllocator::MarkUsed(const Extents& extents) {
    for (auto& e : extents) {
...
bool BitmapAllocator::MarkUsable(const Extents& extents) {
    return false;
}
